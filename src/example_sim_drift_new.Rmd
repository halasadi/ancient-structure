---
title: "Simulations with Drift and Sampling correction"
output: pdf_document
---
...
setwd('/Users/halasadi/ancient-structure/src')
rm(list = ls())
library('data.table')
...
a_freq <- a_freq[2:3,-removeDup(a_freq)]
...
Global parametes: Population size = 1e4 and number of samples from each currenty day population is 100 for a total of 300 populations. And the two ancestral frequencies are from Haak et al, 2015 (Yamanaya and WHG)
...
n_obs = c(24,2);
out <- ancient_structure(geno_data = data, f_obs = t(freq_mat), n_obs = n_obs, K_unknown = K_unknown, max_iter = 100)
...
fa1obs = rbinom(nSNPs,10,fa1)/(10)
fa2obs = rbinom(nSNPs,10,fa2)/(10)
data = sim_ancient_admix(fa1, fa2, t1, t2, alpha, N, nsamp)
...
data = sim_ancient_admix(fa1, fa2, t1, t2, alpha, N, nsamp)
#fa1obs = rnorm(fa1, sd = sqrt(fa1*(1-fa1)/10))
fa1obs = rbinom(length(fa1),10,fa1)/(10)
freq_mat = fa1obs
out <- ancient_structure(geno_data = data, f_obs = t(freq_mat), n_obs = n_obs, K_unknown = K_unknown, max_iter = 100)
barplot(t(out$q),col=2:(K_pooled+1),axisnames=F,space=0,border=NA,main=paste("estd structure: No. of clusters=",K_pooled),
        las=1,ylim=c(0,1),cex.axis=1.5,cex.main=1.4)
