...
ancient_structure <- function(geno_data, f_obs, n_obs, pop_labs, source_labs,K_unknown, max_iter, eps=1e-04, use_squarem=FALSE)
{
  ...
  temp <- fix_clus_mem(t(apply(rev_q_initial, 1,function(x) transform(x))), pop_labs, source_labs);
  ...
  if(!use_squarem)
  {
    for(iter in 1:max_iter) {
      out = update_squarem(param_vec_in, geno_data, f_obs, n_obs, pop_labs, source_labs, nsamp, K_unknown, K_known, nSNPs);
      loglik <- loglik_squarem(param_vec_in, geno_data, f_obs, n_obs, pop_labs, source_labs, nsamp, K_unknown, K_known, nSNPs);
